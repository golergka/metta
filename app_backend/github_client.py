...
class GitHubClient:
    ...
    def get_all_commit_data(self, commit_hash: str, base_branch: str = "main") -> Tuple[str, List[GitCommit], str, str]:
        ...
        total_additions = sum(f.get("additions", 0) for f in commit_data.get("files", []))
        total_deletions = sum(f.get("deletions", 0) for f in commit_data.get("files", []))
        ...
        logger.debug(f"GitHub API: Retrieved all data for {len(commits)} commits, {files_changed} files")
