...
class TrainingRunDescriptionGenerator:
    ...
    def __init__(self, git_client: GitClient, github_client: GitHubClient, llm_client: Optional[LLMClient] = None):
        """
        Initialize the training run description generator.

        Args:
            git_client: Git client for local git operations
            github_client: GitHub client for remote operations
    ...
    def generate_description(self, commit_hash: str, has_uncommitted_changes: bool = False) -> str:
        ...
        try:
            ...
            file_stats = self.git_client.get_range_diff_stats(commit_hash)
            actual_diff = self.git_client.get_range_diff(commit_hash)
        except (GitError, ValueError):
            # Fallback to GitHub API - single optimized call
            try:
                merge_base, commits, file_stats, actual_diff = self.github_client.get_all_commit_data(commit_hash)
            except Exception as e:
                raise ValueError(f"Commit {commit_hash} not found locally or on GitHub") from e

        if not commits:
            raise ValueError(f"No commits found for hash {commit_hash}")

        # Build input data
        input_data = TrainingRunInput(commits=commits, file_stats=file_stats, actual_diff=actual_diff)
    ...
    def _format_user_message(self, input_data: TrainingRunInput) -> str:
        ...
        Args:
            input_data: TrainingRunInput containing commits, file_stats, and actual_diff
        ...
        MAX_DIFF_SIZE = 50000
        actual_diff = input_data.actual_diff
        if len(actual_diff) > MAX_DIFF_SIZE:
...
TRAINING_EXAMPLES: List[tuple[TrainingRunInput, str]] = [
    ...
    (
        TrainingRunInput(
            ...
            file_stats=""" .../learning_progress.yaml}                        |   0
 ...
 8 files changed, 172 insertions(+), 70 deletions(-)""",
            actual_diff="""diff --git a/configs/env/mettagrid/curriculum/navigation/progressive.yaml \\
...
+        self._last_score = None
    ...
    (
        TrainingRunInput(
            ...
            file_stats=""" metta/sweep/README.md                      | 149 ++++++++++
 ...
 16 files changed, 1535 insertions(+)""",
            actual_diff="""diff --git a/metta/sweep/protein.py b/metta/sweep/protein.py
...
+        return math.exp(log_val)""",
    ...
    (
        TrainingRunInput(
            ...
            file_stats=""" configs/agent/fast.yaml              | 9 ++-------
 ...
 5 files changed, 10 insertions(+), 30 deletions(-)""",
            actual_diff="""diff --git a/configs/agent/fast.yaml b/configs/agent/fast.yaml
