...
from app_backend.description_generator import TrainingRunDescriptionGenerator
...
class TestTrainingRunDescriptionGenerator:
    ...
    def test_generator_initialization_no_llm_client(self):
        ...
        with httpx.Client() as http_client:
            ...
            with patch("app_backend.description_generator.logger") as mock_logger:
                TrainingRunDescriptionGenerator(git_client, github_client, None)
                mock_logger.warning.assert_called_once_with("No LLM client available - will fall back to git logs only")

    def test_generator_initialization_with_unavailable_llm_client(self):
        """Test generator initialization with unavailable LLM client."""
        git_client = GitClient()
        with httpx.Client() as http_client:
            github_client = GitHubClient(http_client)
            ...
            with patch.object(llm_client, "is_available", return_value=False):
                with patch("app_backend.description_generator.logger") as mock_logger:
                    TrainingRunDescriptionGenerator(git_client, github_client, llm_client)
                    mock_logger.warning.assert_called_once_with(
                        "No LLM client available - will fall back to git logs only"
                    )
    ...
    def test_generator_initialization_with_available_llm_client(self):
        ...
        with httpx.Client() as http_client:
            ...
            with patch.object(llm_client, "is_available", return_value=True):
                generator = TrainingRunDescriptionGenerator(git_client, github_client, llm_client)
        ...
        with httpx.Client() as http_client:
            ...
            with patch.object(llm_client, "is_available", return_value=True):
                ...
                with (
                    ...
                    patch.object(git_client, "get_range_diff_stats", return_value="2 files changed"),
                    patch.object(git_client, "get_range_diff", return_value="diff content"),
    ...
    def test_generate_description_local_git_success_fallback_to_git_logs(self):
        ...
        with httpx.Client() as http_client:
            ...
            generator = TrainingRunDescriptionGenerator(git_client, github_client, None)
        ...
        with httpx.Client() as http_client:
            ...
            with (
                patch.object(
                    git_client,
                    "get_commit_range",
                    return_value=[GitCommit("abcd1234", "Add new feature", "Author", "2024-01-01")],
                ),
                patch.object(git_client, "get_range_diff_stats", return_value="1 file changed"),
                patch.object(git_client, "get_range_diff", return_value="diff content"),
    ...
    def test_generate_description_github_fallback_success(self):
        ...
        with httpx.Client() as http_client:
            ...
            with (
                patch.object(git_client, "get_commit_range", side_effect=ValueError("Local git failed")),
                patch.object(
                    github_client,
                    "get_all_commit_data",
                    return_value=(
    ...
    def test_generate_description_commit_not_found(self):
        ...
        with httpx.Client() as http_client:
            ...
            with (
                patch.object(git_client, "get_commit_range", side_effect=ValueError("Local git failed")),
                patch.object(github_client, "get_all_commit_data", side_effect=Exception("GitHub failed")),
            ):
                with pytest.raises(ValueError) as exc_info:
                    generator.generate_description("invalid-commit-hash")
                assert "not found locally or on GitHub" in str(exc_info.value)
    ...
    def test_generate_description_no_commits(self):
        ...
        with httpx.Client() as http_client:
            ...
            with patch.object(git_client, "get_commit_range", return_value=[]):
                with pytest.raises(ValueError) as exc_info:
    ...
    def test_build_llm_messages(self):
        ...
        with httpx.Client() as http_client:
            ...
            input_data = TrainingRunInput(
                ...
                file_stats="2 files changed, 10 insertions(+), 5 deletions(-)",
                actual_diff="diff --git a/file.py b/file.py\n+new code",
            )
            ...
            # Check current request message contains our data
            current_message = messages[-1]
            assert current_message["role"] == "user"
            assert "abcd1234" in current_message["content"]
    ...
    def test_build_git_summary_empty(self):
        ...
        with httpx.Client() as http_client:
            ...
            summary = generator._build_git_summary([])

            assert summary == "No commits found in this training run."
    ...
    def test_add_uncommitted_changes_disclaimer(self):
        ...
        with httpx.Client() as http_client:
            ...
            assert result.startswith("COMMIT abcd1234 CONTAINED UNCOMMITTED CHANGES\n\n")
    ...
    def test_generate_description_with_uncommitted_changes(self):
        ...
        with httpx.Client() as http_client:
            ...
            with (
                ...
                assert "COMMIT abcd1234 CONTAINED UNCOMMITTED CHANGES" in result
                assert "Training run based on 1 commit:" in result
    ...
    def test_git_error_converted_to_value_error(self):
        ...
        with httpx.Client() as http_client:
            ...
            # Mock git client to raise GitError, and GitHub to also fail
            with (
                patch.object(git_client, "get_commit_range", side_effect=GitError("Git not installed")),
                patch.object(github_client, "get_all_commit_data", side_effect=Exception("GitHub failed")),
            ):
                with pytest.raises(ValueError) as exc_info:
                    generator.generate_description("invalid-commit-hash")
    ...
    def test_git_not_installed_github_also_fails(self):
        ...
        with httpx.Client() as http_client:
            ...
            # Mock git not being installed, and GitHub also failing
            with (
                patch.object(git_client, "get_commit_range", side_effect=GitError("Git is not installed!")),
                patch.object(github_client, "get_all_commit_data", side_effect=Exception("Network error")),
            ):
                with pytest.raises(ValueError) as exc_info:
                    generator.generate_description("some-commit-hash")
                assert "not found locally or on GitHub" in str(exc_info.value)
